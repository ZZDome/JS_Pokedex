...
async function showPokemons(){
    ...
    for (let id = 0; id < pokemonLenght; id++) {
        if(selectedPokemon == undefined){
            ...
            let response = await fetch(pokemonPATH)
...
/* async function loadPokemonCard(id){
    ...
    showPokemonCard(id, selectedPokemon);
}

function showPokemonCard(id, selectedPokemon){
    let content = document.getElementById('mainContent');
    content = templatePokemonCard(id, selectedPokemon);
} */
...
function templatePokemonCard(id, selectedPokemon){
    return /* html */ `
